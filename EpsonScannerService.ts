...
class EpsonScannerService {
  ...
  async startScan(settings: ScanSettings, options?: { resolution?: number; colorMode?: string; duplex?: boolean }): Promise<ScannedPage[]> {
    console.log('EpsonScannerService: Starting scan with settings:', settings, 'options:', options);
    ...
    const scanOptions = {
      ...settings,
      resolution: options?.resolution || settings.resolution,
      colorMode: options?.colorMode || settings.colorMode,
      duplex: options?.duplex !== undefined ? options.duplex : settings.duplex,
    };

    console.log('EpsonScannerService: Final scan options:', scanOptions);

    const settingsJson = JSON.stringify(scanOptions);
